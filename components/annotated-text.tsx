...
export default function AnnotatedText({ text, sections }: AnnotatedTextProps) {
    ...
    const getHighlightedText = (): JSX.Element[] => {
        ...
        occurrences.forEach((occ, index) => {
            ...
            parts.push(<span key={'text-' + index}>{text.substring(lastIndex, occ.start)}</span>);

            // Add the highlighted phrase with a tooltip containing its description
            parts.push(
                <span key={'highlight-' + index} className="bg-red-200" title={occ.description}>
                    {occ.phrase}
                    {/*TODO: add popover and replace title */}
                </span>
            );
